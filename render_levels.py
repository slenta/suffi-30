...
def load_sprite_image(image_path, size=None):
    ...
    try:
        ...
        if image is None:
            ...
            image = Image.new('RGBA', fallback_size, (255, 0, 255, 255))  # Magenta fallback
...
def draw_items_with_sprites(draw, image, locations, min_x, min_y, sprite_name, size=None):
...
def draw_powerups_with_sprites(draw, image, powerup_locations, min_x, min_y):
    ...
    for powerup in powerup_locations:
        ...
        if powerup_type == 0:  # Size powerup
            ...
            sprite_name = "spraydose.png"  # or whatever sprite you use for speed powerup
...
def draw_enemies_with_sprites(draw, image, enemy_locations, min_x, min_y):
    ...
    for enemy in enemy_locations:
        ...
        draw.line([center_sprite_x - patrol_pixels//2, center_sprite_y,
                  center_sprite_x + patrol_pixels//2, center_sprite_y],
                 fill=(255, 0, 0, 128), width=1)

def draw_special_locations_with_sprites(draw, image, locations, min_x, min_y, sprite_name, fallback_color):
    ...
    for location in locations:
        ...
        try:
            ...
            draw.ellipse([center_x - size//2, center_y - size//2,
                        ...
                        fill=fallback_color, outline=(0, 0, 0), width=3)
...
def render_level(level_config, output_path, show_grid=True):
    ...
    try:
        ...
        draw_items_with_sprites(draw, image, trophy_locations, min_x, min_y, trophy_sprite)
        ...
        exit_location = level_config.get('exit_location')
        if exit_location:
            draw_special_locations_with_sprites(draw, image, [exit_location], min_x, min_y, 
                                              "door_open.png", COLORS['exit'])
        
        # Draw player start position using actual sprites
        player_start = [(5, 1)]  # From PLAYER_START_X, PLAYER_START_Y in settings
        draw_special_locations_with_sprites(draw, image, player_start, min_x, min_y, 
                                          "suffi_00.png", COLORS['player_start'])
...
def draw_legend(draw, width, height):
    ...
    for i, (name, color) in enumerate(legend_items):
        ...
        draw.rectangle([legend_x, y, legend_x + 12, y + 12], fill=color, outline=(0, 0, 0))
...
def draw_level_info(draw, level_config, width):
    ...
    # Background
    draw.rectangle([info_x - 5, info_y - 5,
    ...
    for i, line in enumerate(info_lines):
        ...
        if i < len(colors):
            draw.rectangle([info_x, y, info_x + 12, y + 12], 
                          fill=colors[i], outline=(0, 0, 0))

def main():
    """Main function to render all levels."""
    parser = argparse.ArgumentParser(description='Render platformer levels as PNG images')
    parser.add_argument('level_file', nargs='?', help='Specific level file to render (e.g., level1.py)')
    parser.add_argument('--no-grid', action='store_true', help='Disable grid lines in output')
    parser.add_argument('--output-dir', default='level_renders', help='Output directory for rendered images')
